...
class DerivHandler:  
    def __init__(self):  
        ...
        self.latest_ticks = []
        self.max_ticks = 1000  # Store up to 1000 latest ticks
        ...
        try:
            ...
            def process_tick(response):
                if "tick" in response:
                    ...
                    print(f"🔍 TICK DEBUG - Symbol: {symbol}")
                    ...
                    print(f"   Pip Size: {tick.get('pip_size', 'N/A')}")
                    print(f"   Tick Size: {tick.get('tick_size', 'N/A')}")
                    print(f"   Timestamp: {tick.get('timestamp', 'N/A')}")
                    print(f"   Local Time: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(tick.get('timestamp', time.time())))}")
